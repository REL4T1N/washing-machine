...
from services.booking_service import get_cached_table, delete_booking
...
async def cmd_bookings(message: Message):
    ...
    await show_bookings_menu(message.from_user.id, msg)

@router.callback_query(F.data == "my_bookings")
async def bookings_callback(callback: CallbackQuery):
    """Точка входа через кнопку (если добавишь её в меню)"""
    await callback.answer("🔄 Синхронизация...")
    await show_bookings_menu(callback.from_user.id, callback.message)

@router.callback_query(F.data == "back_to_bookings")
async def back_to_bookings_handler(callback: CallbackQuery):
    """Вернуться к списку (при отмене удаления)"""
    await show_bookings_menu(callback.from_user.id, callback.message)

async def show_bookings_menu(user_id: int, message_obj: Message, page: int = 0):
    """
    ...
    try:
        ...
        user_points = user_storage.sync_user_bookings(user_id, table_data)
...
async def bookings_pagination(callback: CallbackQuery):
    ...
    await show_bookings_menu(callback.from_user.id, callback.message, page)
    await callback.answer()

# --- ВЫБОР ЗАПИСИ ДЛЯ УДАЛЕНИЯ ---
@router.callback_query(F.data.startswith("manage_booking_"))
async def manage_booking_handler(callback: CallbackQuery):
    cell_address = callback.data.replace("manage_booking_", "")
    
    # Проверим, существует ли запись еще
    owner = user_storage.get_owner_by_cell(cell_address)
...
async def confirm_delete_handler(callback: CallbackQuery):
    ...
    success, msg = await delete_booking(cell_address, callback.from_user.id)
